...
/****** Object:  StoredProcedure [dbo].[DASB_RPT]    Script Date: 8/31/2023 9:59:08 AM ******/
DROP PROCEDURE [dbo].[DASB_RPT]
GO
...
CREATE PROCEDURE [dbo].[DASB_RPT](
		@MONTH INT = NULL,
		@YEAR INT = NULL,
		@DefAgency char(2) = NULL,
		@MODE VARCHAR(20)=NULL
) AS BEGIN 

IF(@MODE='AGYLST') BEGIN 
		SELECT DISTINCT DEP_AGENCY, HIE_NAME, (DEP_AGENCY+' - '+HIE_NAME) HIE FROM CASEDEP INNER JOIN  CASEHIE ON
		HIE_AGENCY=DEP_AGENCY AND HIE_DEPT = '' AND HIE_PROGRAM='' WHERE DEP_INTAKE_PROG='Y' ORDER BY DEP_AGENCY
	END

IF(@MODE='AGYRPT') BEGIN
